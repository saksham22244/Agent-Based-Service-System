  ...
  const fetchData = async () => {
    try {
      ...
      rawNotices.forEach(notice => {
        const timeKey = new Date(notice.createdAt).toISOString().substring(0, 16); // if sent in 1 mins then only group same msg
        const key = `${notice.title}|${notice.message}|${notice.recipientType}|${timeKey}`;
        
        if (!groupedMap.has(key)) {
          groupedMap.set(key, { ...notice, groupedIds: [notice.id] }); // Start a new group
        } else { // If the same message already exists, add this notice's ID to the groupedIds array and update recipientName to show it's a group
          const group = groupedMap.get(key); // Add this notice's ID to the existing group
          group.groupedIds.push(notice.id); //  Update the recipientName to show it's a group (e.g. "Multiple (3)")
          group.recipientName = `Multiple (${group.groupedIds.length})`; // Update the recipientName to show it's a group (e.g. "Multiple (3)")
        }
      });
