...
export default function UserServicesPage() {
  ...
  const [dynamicValues, setDynamicValues] = useState({});        // Text field values
  const [fileUploads, setFileUploads] = useState({});            // File upload values
  ...
  useEffect(() => {
    ...
    // Check for search query in URL (e.g., ?search=visa)
  ...
  // Open application modal for selected service
  const handleApplyClick = (service) => {
    setSelectedService(service);
    setDynamicValues({});  // Reset form data
    setFileUploads({});    // Reset file uploads
  };
  ...
  // Update file upload value
  const handleFileChange = (name, file) => {
    setFileUploads(prev => ({ ...prev, [name]: file }));
  };

  // Submit application and redirect to payment
          ...
          <div className="mb-8 bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
             <div className="flex-1 relative">
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition-all text-gray-900 placeholder-gray-500"
                />
                {/* Search icon */}
                  ...
                  services
                    ...
                    .map(service => (
                      <div key={service.id} className="bg-white border hover:shadow-xl transition-shadow border-gray-200 rounded-xl overflow-hidden flex flex-col">
                        {/* Service header with image/icon */}
                        ...
                        {/* Service body with description, price, and apply button */}
